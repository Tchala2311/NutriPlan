...
export async function getUserSettings(): Promise<UserSettings> {
  ...
  if (!data) {
    return {
      ...DEFAULT_SETTINGS,
      dietary_restrictions:
        (health?.dietary_restrictions as string[]) ?? DEFAULT_SETTINGS.dietary_restrictions,
  ...
  return {
    ...
    budget_preference:
      (data.budget_preference as UserSettings['budget_preference']) ??
      DEFAULT_SETTINGS.budget_preference,
    dietary_restrictions:
      (health?.dietary_restrictions as string[]) ?? DEFAULT_SETTINGS.dietary_restrictions,
    allergens: (health?.allergens as string[]) ?? DEFAULT_SETTINGS.allergens,
  };
...
export async function saveSettings(formData: FormData) {
  ...
  const settings = {
    ...
    notification_prefs: {
      ...
      ai_suggestion_timing: (formData.get('ai_suggestion_timing') as string) || 'off',
    },
  ...
  if (existing) {
    ...
    const result = await supabase.from('user_settings').update(settings).eq('id', existing.id);
    settingsError = result.error;
  } else {
    // Row doesn't exist, insert new row
    const result = await supabase.from('user_settings').insert([settings]);
    settingsError = result.error;
  }
