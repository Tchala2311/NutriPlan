...
export async function getUserGoals(): Promise<UserGoals> {
  ...
  const { data: goals } = await supabase
    .from('user_goals')
    .select(
      'primary_goal, daily_calorie_target, protein_target_g, carbs_target_g, fat_target_g, weight_kg, height_cm, age, sex, activity_level'
    )
    .eq('user_id', user.id)
    .maybeSingle();
  ...
  if (goals) {
    ...
    if (tdee) {
      const computed = calculateMacros(
        tdee,
        goals.primary_goal ?? 'general_wellness',
  ...
  return {
    ...
    protein_target_g:
      assessment?.protein_target_g ?? Math.round(base.protein_target_g * calorieRatio),
...
export async function getTrendsData(goals: UserGoals): Promise<TrendsData> {
  ...
  const byDate = new Map<
    string,
    { calories: number; protein_g: number; carbs_g: number; fat_g: number; foods: string[] }
  >();
...
export async function saveUserGoals(formData: FormData) {
  ...
  const weight_kg = formData.get('weight_kg')
    ? parseFloat(String(formData.get('weight_kg')))
    : null;
  const height_cm = formData.get('height_cm')
    ? parseInt(String(formData.get('height_cm')), 10)
    : null;
  ...
  const goals = {
    ...
    daily_calorie_target:
      macros?.daily_calorie_target ?? Number(formData.get('daily_calorie_target') ?? 2000),
  ...
  if (existing) {
    ...
    const { error: insertError } = await supabase.from('user_goals').insert([goals]);
...
export async function saveUserPreferences(formData: FormData): Promise<{ error?: string }> {
  ...
  const { error: healthError } = await supabase
    .from('health_assessments')
    .upsert(
      { user_id: user.id, dietary_restrictions: dietaryRestrictions, allergens },
      { onConflict: 'user_id' }
    );
