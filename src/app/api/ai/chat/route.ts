...
export async function POST(req: NextRequest) {
  ...
  const { data: assessment } = await supabase
    ...
    .select(
      'primary_goal, dietary_restrictions, allergens, medical_conditions, eating_disorder_flag, eating_disorder_anorexia_restrictive, eating_disorder_binge, eating_disorder_orthorexia, secondary_goals, is_pregnant, pregnancy_trimester, is_breastfeeding'
    )
    .eq('user_id', user.id)
