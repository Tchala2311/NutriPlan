...
export async function POST(req: Request) {
  ...
  if (existingPlan?.slots) {
    for (const [date, daySlots] of Object.entries(
      existingPlan.slots as Record<string, Record<string, { recipe_id: string; pinned: boolean }>>
    )) {
  ...
  const { data: recipes } = await supabase
    .from('recipes')
    .select(
      'id, title, prep_time_min, calories_per_serving, protein_per_serving, carbs_per_serving, fat_per_serving, ingredients, instructions, dietary_tags, substitutions'
    )
    .in('id', [...allRecipeIds]);
