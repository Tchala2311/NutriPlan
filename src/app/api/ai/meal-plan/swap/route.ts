...
export async function POST(req: Request) {
  ...
  const daySlots = (plan.slots as Record<string, Record<string, { recipe_id: string; pinned: boolean }>>)[date] ?? {};
  ...
  function per100g(v: number) { return Math.round((v / weightEstimate) * 100 * 10) / 10; }
