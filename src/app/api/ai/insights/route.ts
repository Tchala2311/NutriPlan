...
import { detectSafetyAlert, detectTrendWarning, detectPlateau } from '@/lib/nutrition/detection';
...
export async function POST(req: NextRequest) {
  ...
  const [{ data: assessment }, { data: settings }, { data: goals }] = await Promise.all([
    supabase
      ...
      .limit(1)
      .single(),
    supabase.from('user_settings').select('tone_mode').eq('user_id', user.id).single(),
    supabase
      .from('user_goals')
  ...
  try {
    ...
    switch (type) {
      case 'daily_analysis': {
        result = await getDailyAnalysis(profile, payload as Parameters<typeof getDailyAnalysis>[1]);
        break;
      }
      case 'safety_alert': {
      ...
      case 'goal_insight': {
        ...
        if (profile.primary_goal === 'weight_loss' && !goalPayload.plateau_detected) {
          ...
          const plateau = detectPlateau(weights, nutrition, goals?.daily_calorie_target ?? 2000);
