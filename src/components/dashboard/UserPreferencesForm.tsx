...
export function UserPreferencesForm({
  ...
  const [dietaryRestrictions, setDietaryRestrictions] = useState<Set<string>>(new Set(initialDietaryRestrictions));
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="rounded-xl border border-parchment-200 bg-parchment-100 p-6">
        <h2 className="text-sm font-semibold text-bark-300 uppercase tracking-wide mb-5">Тренировочные дни</h2>
        <div>
          <label className="block text-sm font-medium text-bark-300 mb-1.5">Дни тренировок в спортзале</label>
      ...
      {/* Food Preferences */}
      <div className="rounded-xl border border-parchment-200 bg-parchment-100 p-6">
        <h2 className="text-sm font-semibold text-bark-300 uppercase tracking-wide mb-5">Пищевые предпочтения</h2>
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-bark-300 mb-1.5">Диетические ограничения</label>
