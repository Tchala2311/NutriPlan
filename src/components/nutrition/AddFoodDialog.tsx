...
export function AddFoodDialog({ date, defaultMeal, children, onAdded }: AddFoodDialogProps) {
  ...
  const estimateViaGigaChat = useCallback(
    async (id: number) => {
      ...
      try {
        ...
        setCustomIngredients((prev) =>
          prev.map((r) =>
            r.id === id
              ? { ...r, loading: false, error: err instanceof Error ? err.message : 'Ошибка' }
              : r
          )
        );
      }
  ...
  const resolvedCustom = customIngredients
    .map((r) => r.resolved)
    .filter(Boolean) as FoodPhotoItem[];
        ...
        <Dialog.Content className="fixed left-1/2 top-1/2 z-50 w-full max-w-md -translate-x-1/2 -translate-y-1/2 rounded-xl bg-parchment-50 border border-parchment-200 p-6 shadow-xl data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 max-h-[90vh] overflow-y-auto">
          ...
          <div className="mb-5">
            ...
            {photoState === 'uploading' && photoLong && (
              <p className="mt-1.5 text-center text-xs text-muted-foreground animate-pulse">
                Ещё немного…
              </p>
            )}
              ...
              <div className="mt-3 space-y-3">
                ...
                {apology && (
                  <div className="rounded-lg bg-amber-50 border border-amber-200 px-3 py-2 text-xs text-amber-800">
                    ...
                    {apology} Проверьте данные перед сохранением.
                  </div>
                )}
                      ...
                      >
                        ...
                        <span className="flex-1">
                          ...
                          <span className="ml-1.5 text-bark-200">
                            {Math.round(item.calories)} ккал · Б{item.protein_g.toFixed(1)} Ж
                            {item.fat_g.toFixed(1)} У{item.carbs_g.toFixed(1)}
                          </span>
                        </span>
                      </label>
                    ))}
          ...
          <form ref={formRef} onSubmit={handleSubmit} className="space-y-4">
            ...
            {!hasPhotoItems && (
              <>
                ...
                <div className="grid grid-cols-3 gap-3">
                  {(
                    [
                      ['Белки (г)', manualP, setManualP],
                      ['Углеводы (г)', manualC, setManualC],
                      ['Жиры (г)', manualF, setManualF],
                    ] as const
                  ).map(([label, val, set]) => (
                    <div key={label}>
                      <label className="block text-xs font-medium text-bark-200 mb-1">
            ...
            {customIngredients.map((row) => (
              <CustomIngredientRow
                ...
                onHideSuggestions={() =>
                  setCustomIngredients((prev) =>
                    prev.map((r) => (r.id === row.id ? { ...r, showSuggestions: false } : r))
                  )
                }
                onSetSuggestions={(suggestions, loading) =>
                  setCustomIngredients((prev) =>
                    prev.map((r) =>
                      r.id === row.id ? { ...r, suggestions, suggestionsLoading: loading } : r
                    )
                  )
                }
            ...
            {allItems.length > 0 && (
              <div className="rounded-lg bg-bark-300 text-primary-foreground px-4 py-3 text-sm">
                <div className="font-medium mb-1">
                  ...
                  {allItems.length > 1 && (
                    <span className="text-xs opacity-70 ml-1">({allItems.length} поз.)</span>
                  )}
                </div>
...
function CustomIngredientRow({
  ...
  useEffect(() => {
    ...
    fetch(`/api/ingredients?q=${encodeURIComponent(debouncedName)}`)
      ...
      .catch(() => {
        if (!cancelled) onSetSuggestions([], false);
      });
    return () => {
      cancelled = true;
    };
      ...
      {row.resolved && (
        <div className="text-xs bg-sage-50 rounded px-2 py-1.5 space-y-0.5">
          <div className="font-medium text-sage-700">
            {row.resolved.food_name} · {row.resolved.portion}
          </div>
          <div className="text-sage-600">
            {Math.round(row.resolved.calories)} ккал · Б{row.resolved.protein_g.toFixed(1)}г Ж
            {row.resolved.fat_g.toFixed(1)}г У{row.resolved.carbs_g.toFixed(1)}г
            {row.resolved.weight_was_estimated && (
              <span className="ml-1.5 text-amber-600">
                (вес оценён ИИ ~{row.resolved.weight_g}г)
              </span>
            )}
          </div>
        </div>
